...
def classify_event(event: Event, profile: dict, recurrence: RecurrenceEvidence | None = None):
    ...
    lifecycle = event.status in {"cancelled","failed","unrealized"} or event.direction=="non_cash"
    if lifecycle: evidence_class="UNKNOWN"
    elif protected: evidence_class="PROTECTED" if event.category in protected_categories else "FIXED_REQUIRED"
    elif recurrence is not None and recurrence.amount_estimate is not None:
        evidence_class="RECURRING_REQUIRED" if not flexible else "RECURRING_OPTIONAL"
    elif recurrence is not None: evidence_class="UNSUPPORTED_RECURRING"
    elif event.status in {"settled","pending","scheduled"}: evidence_class="ONE_TIME"
    else: evidence_class="UNKNOWN"
    return {"protected": protected, "flexible": flexible, "optional": optional,
            "fixed": fixed, "modifiable": modifiable,
            "evidence_class": evidence_class,
            "provenance": {"event_id":event.event_id,"category":event.category,
                           "description":event.description,"status":event.status,
                           "recurrence_ids":list(recurrence.provenance) if recurrence else []}}
...
OPTIONAL_BASELINE_POLICIES={"INCLUDE_REQUIRED_ONLY","INCLUDE_ALL_RECURRING","EXCLUDE_OPTIONAL_FLEXIBLE","CURRENT_BEHAVIOR"}

def baseline_include_optional(query):
    """Resolve the explicit optional-baseline experiment policy."""
    policy=getattr(DEFAULT_POLICY,"optional_baseline_policy","CURRENT_BEHAVIOR")
    if policy not in OPTIONAL_BASELINE_POLICIES:
        raise ValueError(f"unsupported optional baseline policy: {policy}")
    # Preserve the pre-experiment split by default: safe amount excluded
    # optional flexible spending, earliest-date scan used the full ledger.
    if policy=="CURRENT_BEHAVIOR": return query=="earliest"
    return policy=="INCLUDE_ALL_RECURRING"
...
def safe_amount(user, req, data, changes=()):
    ...
    include_optional=baseline_include_optional("safe")
    ok, bal=forecast(user,rd,data,((rd,requested),),changes,include_optional=include_optional)
    lo=ZERO; hi=requested
    for _ in range(80):
        mid=((lo+hi)/2).quantize(CENT, rounding=ROUND_HALF_EVEN)
        if mid<=lo: break
        good,_=forecast(user,rd,data,((rd,mid),),changes,include_optional=include_optional)
        if good: lo=mid
        else: hi=mid
    return lo
...
def earliest(user, req, data, changes=()):
    ...
    d=rd
    while d<=end:
        good,_=forecast(user,rd,data,((d,amount),),changes,include_optional=baseline_include_optional("earliest"))
        if good: return d
        d+=timedelta(days=1)
    return None
